...
def update_google_sheet_base(credentials_filename, sheet_name, ticker):
	...
	try:
		...
		df_zig1 = None
		try:
			worksheet = sh.worksheet("zig1-" + ticker)
		...
		df_zig15 = None
		try:
			worksheet = sh.worksheet("zig15-" + ticker)
		...
		df_zig2 = None
		try:
			worksheet = sh.worksheet("zig2-" + ticker)
		...
		print("[-] update_google_sheet_base: error: gspread.exceptions.APIError")

def update_google_sheet_mtf(dataframe, credentials_filename, sheet_name, ticker):
	#Authorize the API
	scope = [
		'https://www.googleapis.com/auth/drive',
		'https://www.googleapis.com/auth/drive.file'
	...
	try:
		...
		print("[-] update_google_sheet_mtf:  error: gspread.exceptions.APIError")
...
def update_google_sheet_mtf_dialy(dataframe, credentials_filename, ticker):
	...
	try:
		...
		print("[-] update_google_sheet_mtf_dialy: error: gspread.exceptions.APIError")
...
def hello_http(request):
	...
	if content_type == 'text/plain; charset=utf-8':
		...
		if signal == "mtf":
			...
			update_google_sheet_mtf(output_df, credentials_filename, sheet_name, ticker)
