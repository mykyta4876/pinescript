...
#python get_option_data_by_alarm_flask_MD2R_FirstPrice.py -m 0.001
...
backend_address = "http://34.84.31.169"
...
def get_signal(df, Multiplier, XBarsDelay, adxlen, start_price, start_time, code_name):
    deviation1 = 1.0
    deviation2 = 2.0
    trading_time = True
    ...
    output_df = df[['time', 'open', 'high', 'low', 'close', 'buy', 'sell', 'netted_long_position', 'netted_short_position', 'netted_long_avg', 'netted_short_avg', 'buy_profit_cut', 'sell_profit_cut', 'buy_profit_cutoff', 'sell_profit_cutoff', 'buy_profit', 'sell_profit', 'lastDotAbove1Open', 'lastDotBelow1Open', 'upArrow2', 'dnArrow2', 'upArrow2r', 'dnArrow2r']]
    output_filename = code_name + "_calc.csv"
...
def get_signal_thread(quote_ctx, alert_json, code_name, Multiplier, XBarsDelay, adxlen):
    ...
    bFlag, df = get_option_data(quote_ctx, code_name, 1000)
    ...
    while True:
        ...
        if hour == 8 or (hour == 9 and minute > 20 and minute < 25):
            break
        ...
        while True:
            ...
            last_time = datetime.now() + timedelta(minutes=1)
            bFlag, df1 = get_option_data(quote_ctx, code_name, 1)
            ...
            output_df = df_merge_outer[['time', 'open', 'high', 'low', 'close']]
            output_filename = code_name + ".csv"
            output_df.to_csv(output_filename, index=False)

            entry_signal_code = get_signal(df_merge_outer, Multiplier, XBarsDelay, adxlen, start_price, start_time, code_name)
        ...
        if position_type == entry_signal_code:
            continue
            
        """
        if position_type == "":
            if entry_signal_code == "Buy" and start_price > df1['close'].iloc[0]:
                continue
            if entry_signal_code == "Sell" and start_price < df1['close'].iloc[0]:
                continue
        """
        ...
        if len(position_type) == 0 and entry_signal_code == "BUY":
            continue

        position_type = entry_signal_code
        result_json['side'] = entry_signal_code
        result_json['x-time'] = df1['time_key'].iloc[0]
        result_json['x-price'] = start_price
        result_json['code_name'] = code_name
        ...
        try:
            ...
            response = requests.post(backend_address, data=json_data, headers={'Content-Type': 'application/json'})
...
def parse_arguments():
    ...
    parser.add_argument("--delay", "-d", help="delay", type=int, default=1)
...
def hello_http():
    ...
    if request_json and "symbol" in request_json and "option" in request_json:
        ...
        thread1 = threading.Thread(target=get_signal_thread, args=(quote_ctx, request_json, code_name, Multiplier, XBarsDelay, adxlen))
        thread1.start()
