...
def calculate_trading_signals(df_group, max_positions):
    ...
    for i in range(len(df_group)):
        ...
        # Long entry
        if len(long_positions) < max_positions and is_bull_bar and current_bar['close'] > current_bar['vww']:
            long_positions.append((current_bar['close'], current_bar['open']))
            if not first_long_entry_open:  # Only set first entry open price if it's not set
                first_long_entry_open = current_bar['open']
            df_group.at[df_group.index[i], 'Long_Entry_Price'] = current_bar['open']
        
        # Short entry
        if len(short_positions) < max_positions and is_bear_bar and current_bar['close'] < current_bar['vww']:
            short_positions.append((current_bar['close'], current_bar['open']))
            if not first_short_entry_open:  # Only set first entry open price if it's not set
                first_short_entry_open = current_bar['open']
...
def process_option_data(results, strike_price, expiry_date, max_positions):
    ...
    df = calculate_trading_signals(df, max_positions)
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python engine.py <max_positions> <input_excel_file>")
        return
    ...
    try:
        ...
        for idx, row in df_input.iterrows():
            ...
            # Process results
            if results:
                df_results = process_option_data(results, strike_price, expiry_date, max_positions)
