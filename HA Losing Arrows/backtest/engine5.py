...
def main():
    ...
    try:
        ...
        for i in range(len(df_input) - 1):
            ...
            if len(long_positions) < max_positions and is_bull_bar and current_bar['close'] > current_bar[vwap_column] and not exit3_happened:
                long_pending_entry = True
            
            if len(short_positions) < max_positions and is_bear_bar and current_bar['close'] < current_bar[vwap_column] and not exit3_happened:
