...
def calculate_trading_signals(df_group, max_positions, vwap_type, use_exit1):
    """Calculate trading signals with configurable VWAP and exit rules"""
    ...
    for i in range(len(df_group) - 1):
        ...
        # Only check Exit1 if enabled
        if use_exit1 and long_positions and current_bar['close'] < first_long_entry_open:
        ...
        if pending_entry and len(long_positions) < max_positions:
        ...
        # Check for entry conditions using selected VWAP
    ...
    if long_positions:
        df_group.at[df_group.index[-1], 'Long_Balance'] += df_group.at[df_group.index[-2], 'Long_Equity']
...
def process_option_data(results, strike_price, expiry_date, max_positions, vwap_type, use_exit1):
    ...
    columns = ['timestamp', 'vw', 'vww', 'volume', 'open', 'close', 'high', 'low',
              ...
              'Long_Positions', 'Long_Entry_Price']
...
def main():
    if len(sys.argv) < 6:
        print("Usage: python engine2_long.py <max_positions> <timeframe> <vwap_type> <use_exit1> <input_excel_file>")
        print("Example: python engine2_long.py 10 10m vw n 'SPY C weekly.xlsx'")
        return
    ...
    try:
        ...
        if all_results:
            ...
            output_file = f"{input_file_name.split('.')[0]}_output_long.xlsx"
