        ...
        try:
            ...
            for idx, row in tqdm(df.iterrows(), total=len(df), desc="Processing rows"):
                ...
                if cache_key in self.cached_data:
                    ...
                    #print(option_data['timestamp'])
                    #print(row['start'])
                    # Find the open price at start time
                    start_data = option_data[option_data['timestamp'] >= row['start']].iloc[0]
                    #print(start_data)
                    #input()
                    df.at[idx, 'Open_Price'] = start_data['open']
                    
                    # Calculate target price
                    target_price = start_data['open'] * self.multiplier
                    ...
                    if not exceeded_target.empty:
                        ...
                        df.at[idx, 'Exit_Price'] = last_price['close']
                        df.at[idx, 'Exit_Time'] = last_price['timestamp']
            
            # Generate output filename
            input_name = self.input_file.split('.')[0]
            output_file = f"{input_name}_{self.timeframe}{self.option_type}{self.multiplier}.xlsx"
            
            # Select only needed columns
            result_df = df[['start', 'strike', 'expiry', 'Open_Price', 'Exit_Price', 'Exit_Time']]
...
def main():
    if len(sys.argv) != 5:
        print("Usage: python engine7.py <timeframe> <option_type> <multiplier> <input_file>")
        print("Example: python engine7.py 1m C 2 'SPY_Weekly.xlsx'")
        return
