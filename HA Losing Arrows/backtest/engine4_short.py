...
def main():
    if len(sys.argv) < 7:
        print("Usage: python engine2.py <max_positions> <reverse_entry> <vwap_type> <use_exit1> <stop_loss_percentage> <reverse_exit> <input_excel_file>")
        print(f"Example: python engine4.py 10 r vw y 0.2 r \"SPY C weekly.xlsx\"")
        return
    ...
    try:
        ...
        for i in range(len(df_input) - 1):
            ...
            if mininum_entry_price == 0 or current_bar['close'] > mininum_entry_price:
                if len(short_positions) < max_positions and is_bear_bar:
        ...
        output_file = f"{input_file_name.split('.')[0]}_short_{settings}.xlsx"
