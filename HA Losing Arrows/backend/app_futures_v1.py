...
order_pair_map = {}
futures_naked_pending_orders = {}
futures_naked_exit_todo_orders = {}
    ...
    while True:
        try:
            ...
            for api_key, api_info in api_keys.items():
                ...
                # confirm the pending futures naked orders
                futures_naked_pending_orders_copy = futures_naked_pending_orders.copy()
                
                processed_list = []
                for p_order_id, p_order in futures_naked_pending_orders_copy.items():
                    ...
                    for l_order in json_object_order_list:
                        if l_order['order_id'] == p_order_only_id and l_order['order_status'] == "FILLED_ALL":
                            logger.info(f"[-] thread_exit_orders: Futures NAKED: pending order {p_order_id} is FILLED_ALL")
                            orders[p_order_id] = p_order
                            processed_list.append(p_order_id)
                            break

                for order_id in processed_list:
                    if order_id in futures_naked_pending_orders:
                        del futures_naked_pending_orders[order_id]

                # place the exit orders for futures naked orders
                futures_naked_exit_todo_orders_copy = futures_naked_exit_todo_orders.copy()
                futures_naked_exit_todo_orders_copy_merged = merge_orders(futures_naked_exit_todo_orders_copy, 10)
                for order_id, order in futures_naked_exit_todo_orders_copy.items():
                    del futures_naked_exit_todo_orders[order_id]
                
                for order_id, order in futures_naked_exit_todo_orders_copy_merged.items():
                    futures_naked_exit_todo_orders[order_id] = order
                ...
                for order_id, order in futures_naked_exit_todo_orders_copy.items():
                    ...
                    if order["api_key"] != api_key:
                        continue
                    logger.info(f"[-] thread_exit_orders: Futures NAKED: Exiting Order: ID: {order_id}, Details: {order}")
                    if order["side"] == "BUY":
                        order['side'] = "SELL"
                    else:
                        order['side'] = "BUY"
                    ...
                    try:
                        ...
                        if "Open position failed" in str(response_json):
                            logger.error(f"[-] thread_exit_orders: Futures NAKED: Open position failed: {response_json}, acc:{api_info['id']}, order:{order}")
                            continue

                        try:
                            json_object = json.loads(response_json)
                        except Exception as e:
                            logger.error(f"[-] thread_exit_orders: Futures NAKED: error: {e}, acc:{api_info['id']}, order:{order}")
                            continue

                        if "order_id" in json_object[0]:
                            logger.info(f"[-] thread_exit_orders: Futures NAKED: Placed the pending exit order of {order_id} successfully. ID: {json_object[0]['order_id']}, acc:{api_info['id']}, order:{order}")
                            processed_list.append(order_id)
                        else:
                            logger.error(f"[-] thread_exit_orders: Futures NAKED: Order response missing 'order_id', acc:{api_info['id']}, order:{order}")

                    except requests.exceptions.RequestException as e:
                        logger.error(f"[-] thread_exit_orders: Futures NAKED: FastAPI request failed: {e}, acc:{api_info['id']}, order:{order}")
                    ...
                    for l_order in json_object_order_list:
                        if l_order['order_id'] == p_order_only_id and l_order['order_status'] == "FILLED_ALL":
                            ...
                            if "try_count" in p_order and p_order["try_count"] > 10:
                                logger.error(f"[-] sell_order: try_count is greater than 10, acc:{api_info['id']}, order:{p_order}")
                                processed_list.append(p_order_id)
                                break
                ...
                logger.info(f"[-] thread_exit_orders: {api_info['id']} Sell finished")
...
def exit_positions(data, symbol, action, api_key, ordertag):
    ...
    # Futures Naked Strategy
    if ordertag == 1:
        copy_orders = orders.copy()
        to_exit_orders = {
            order_id: order for order_id, order in copy_orders.items()
            if order['symbol'] == symbol and ((option == "C" and order['side'] == "BUY") or (option == "P" and order['side'] == "SELL")) and order['api_key'] == api_key and order['order_tag'] == ordertag
        }
...
def forward_order1(data, api_key):
    ...
    ordertag = int(data.get("order_tag", 0))
    ...
    if ordertag == 1:
        ...
        try:
            ...
            if "Open position failed" in str(response_json):
                logger.error(f"forward_order: Futures Naked: Open position failed: {response_json} : {code_name}")
                return jsonify({"error": "Open position failed"}), 500

            try:
                json_object = json.loads(response_json)
            except Exception as e:
                logger.error(f"[-] forward_order: Futures Naked: error {e} : {code_name}")
                return jsonify({"error": str(e)}), 500

            order_id = json_object[0]["order_id"] + api_keys[api_key]["id"]
            futures_naked_pending_orders[order_id] = order_data

            logger.info(f"[-] forward_order: Futures Naked: Buy pending order placed. Order ID: {order_id} : {code_name}")
