...
def check_exited_sell_orders(exit_sell_pending_orders, buy_to_exit_orders):
    ...
    while True:
        ...
        try:
            ...
            try:
                ...
                logger.error(f"[-] check_exited_sell_orders: error: {e}")
                continue
            ...
            for p_order_id, p_order in exit_sell_pending_orders_t.items():
                for l_order in json_object:
                    if l_order['order_id'] == p_order_id and l_order['order_status'] == "FILLED_ALL":
                        ...
                        logger.info(f"[-] check_exited_sell_orders: {p_order_id} is FILLED_ALL")
        ...
        if len(exit_sell_pending_orders_t) == 0:
            logger.info("[-] check_exited_sell_orders: confirmed that exited all sell pending orders")
            break

    exit_buy_pending_orders = {}
    for order_id, order in buy_to_exit_orders.items():
        logger.info(f"[-] check_exited_sell_orders: Exiting Order: ID: {order_id}, Details: {order}")
        order['side'] = "SELL"
        ...
        try:
            ...
            if "Open position failed" in str(response_json):
                logger.error(f"[-] check_exited_sell_orders: Open position failed (BUY): {response_json}")
                continue

            try:
                json_object = json.loads(response_json)
            except Exception as e:
                logger.error(f"[-] check_exited_sell_orders: BUY: error {e} : {order}")
                return retry_buy_order(fixed_multiple - 1)

            if "order_id" in json_object[0]:
                logger.info(f"[-] check_exited_sell_orders: Exited BUY Order {order_id} successfully. ID: {json_object[0]['order_id']}")
                order_id = json_object[0]['order_id']
                exit_buy_pending_orders[order_id] = order
            else:
                logger.error("[-] check_exited_sell_orders: Order response missing 'order_id'")
        except requests.exceptions.RequestException as e:
            logger.error(f"[-] check_exited_sell_orders: FastAPI request failed: {e}")
    ...
    while True:
            ...
            for p_order_id, p_order in exit_buy_pending_orders.items():
                for l_order in json_object:
                    if l_order['order_id'] == p_order_id and l_order['order_status'] == "FILLED_ALL":
                        ...
                        logger.info(f"[-] check_exited_sell_orders(buy): {p_order_id} is FILLED_ALL")
        ...
        if len(exit_buy_pending_orders) == 0:
            logger.info("[-] check_exited_sell_orders: confirmed that exited all buy pending orders")
            break
...
def exit_positions(data, symbol, action, api_key):
    ...
    executor.submit(check_exited_sell_orders, exit_sell_pending_orders, buy_to_exit_orders)
