...
def forward_order():
    ...
    def retry_buy_order(fixed_multiple):
        ...
        try:
            ...
            orders[order_id] = buy_order_data
            logger.info(f"Buy order placed. Order ID: {order_id} : {code_name} : {fixed_multiple}")
    ...
    try:
        ...
        try:
            ...
            logger.error(f"[-] forward_order: SELL: error {e} : {sell_code_name}")
            return jsonify(sell_response_json), sell_response.status_code

        order_id = json_object[0]["order_id"]
        orders[order_id] = sell_order_data
        logger.info(f"Sell order placed. Order ID: {order_id} : {sell_code_name}")
        return jsonify(sell_response_json), sell_response.status_code
    except requests.exceptions.RequestException as e:
        logger.error(f"Sell order failed: {e} : {sell_code_name}")
        return jsonify({"error": str(e)}), 500
