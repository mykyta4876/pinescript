...
os.environ["Flag_Thread_Exit_Orders"] = "False"
...
def thread_exit_orders():
    ...
    if os.environ["Flag_Thread_Exit_Orders"] == "True":
        return

    os.environ["Flag_Thread_Exit_Orders"] = "True"

    now_time = datetime.now().astimezone(pytz.timezone('US/Eastern'))

    logger.info(f"[-] thread_exit_orders: now_time: {now_time} start")
    if now_time.hour < 9:
        logger.info(f"[-] thread_exit_orders: now_time: {now_time} end because of hour < 9")
        os.environ["Flag_Thread_Exit_Orders"] = "False"
        return

    if now_time.hour == 9 and now_time.minute < 30:
        logger.info(f"[-] thread_exit_orders: now_time: {now_time} end because of hour == 9 and minute < 30")
        os.environ["Flag_Thread_Exit_Orders"] = "False"
        return

    if now_time.hour > 15:
        logger.info(f"[-] thread_exit_orders: now_time: {now_time} end because of hour > 15")
        os.environ["Flag_Thread_Exit_Orders"] = "False"
        return
    
    if now_time.weekday() >= 5:
        logger.info(f"[-] thread_exit_orders: now_time: {now_time} end because of weekday >= 5")
        os.environ["Flag_Thread_Exit_Orders"] = "False"
        return
    ...
    for api_key, api_info in api_keys.items():
        ...
        if counts == 0:
            logger.info(f"[-] thread_exit_orders: now_time: {now_time} end because of {api_info['id']} has no orders")
            continue
                ...
                if l_order['order_id'] == p_order_only_id and l_order['order_status'] == "FILLED_ALL":
                    ...
                    try:
                        ...
                        try:
                            ...
                            logger.error(f"[-] sell_order: error: {e}, sell order of {p_order_id}, acc:{api_info['id']}, order:{p_order}, response:{sell_response_json}")
                            break

                        new_order_id = sell_json_object[0]["order_id"] + api_keys[api_key]["id"]
                        orders[p_order_id] = buy_pending_orders[p_order_id]
                        orders[new_order_id] = p_order
                        order_pair_map[p_order_id] = new_order_id
        ...
        for order_id, order in exit_sell_todo_orders.items():
            ...
            try:
                ...
                if "order_id" in json_object[0]:
                    ...
                    exit_sell_pending_orders[new_order_id] = order
                else:
        ...
        for order_id, order in exit_buy_todo_orders.items():
            ...
            try:
                ...
                if "order_id" in json_object[0]:
                    ...
                    processed_list.append(order_id)
                    new_order_id = json_object[0]['order_id'] + api_keys[api_key]["id"]
                    exit_buy_pending_orders[new_order_id] = order
                else:
    ...
    flag_thread_exit_orders = False

    logger.info(f"[-] thread_exit_orders: now_time: {now_time} end successfully")

@app.route("/call_thread_exit_orders", methods=["POST"])
def call_thread_exit_orders():

    data = request.json
    api_key = data.get("api_key")
    if not api_key:
        logger.error(f"[-] call_thread_exit_orders: Missing API key")
        return jsonify({"error": "Missing API key"}), 403

    if api_key != "1234567890":
        logger.error(f"[-] call_thread_exit_orders: Invalid API key: {api_key}")
        return jsonify({"error": "Invalid API key"}), 403

    if os.environ["Flag_Thread_Exit_Orders"] == "True":
        logger.info("[-] call_thread_exit_orders: thread_exit_orders is already running")
        return jsonify({"success": "Thread exit orders already running"}), 200
    
    executor.submit(thread_exit_orders)
    return jsonify({"success": "Thread exit orders called"}), 200
...
def list_memory_orders():
    ...
    memory_orders = {}
    memory_orders['sell_todo_orders'] = sell_todo_orders
