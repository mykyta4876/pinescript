...
def thread_engin4_long(symbol, expiry_date, option_type, strike_price, vwap_column, stop_loss_percentage, reverse_exit, max_positions, reverse_entry, use_exit3, api_key, bar_size, mininum_entry_price):
    """Main loop to run the live trading engine"""
    print(f"Starting live trading for {format_option_ticker(symbol, expiry_date, option_type, strike_price)}")
...
def get_ask_price(code_name, api_key):
    ...
    while True:
        try:
            response = requests.post(opend_address + "/get_ask_price", json=json_params, headers=headers)
            response.raise_for_status()
            response_json = response.json()
...
def forward_order1(data, api_key):
    ...
    if bar_size.find("m") != -1:
        ...
        reverse_exit = True if reverse_exit == "true" else False
        max_positions = int(data.get("Max positions:", 0))
        reverse_entry = data.get("EntryReverse", "")
        reverse_entry = True if reverse_entry == "true" else False
        use_exit3 = data.get("UseExit3", "")
        use_exit3 = True if use_exit3 == "true" else False
        mininum_entry_price = float(data.get("MininumEntryPrice", 0))
        threading.Thread(target=thread_engin4_long, args=(symbol, datetmp, option_value, strikeprice, vwap_column, stop_loss_percentage, reverse_exit, max_positions, reverse_entry, use_exit3, api_key, bar_size, mininum_entry_price)).start()
    ...
    def retry_buy_order(fixed_multiple):
        ...
        try:
            ...
            logger.error(f"Buy order failed: {e} : {code_name}, {buy_order_data}")
            return jsonify({"error": str(e)}), 500
