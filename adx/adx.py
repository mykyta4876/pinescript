...
def calculate_adx_di(df, length=14):
    ...
    df['DM+_smoothed'] = 0.0 
    df['DM-_smoothed'] = 0.0
    df['TR_smoothed'] = df['TR'].rolling(window=length).mean()
    df['DM+_smoothed'] = df['DM+'].rolling(window=length).mean()
    df['DM-_smoothed'] = df['DM-'].rolling(window=length).mean()

    """
    df['TR_smoothed'] = df['TR'].ewm(span=length, adjust=False).mean()
    df['DM+_smoothed'] = df['DM+'].ewm(span=length, adjust=False).mean()
    df['DM-_smoothed'] = df['DM-'].ewm(span=length, adjust=False).mean()
    """
...
def plot_adx_di(df,Threshold):
    ...
    # Plot DI+ and DI-
    plt.plot(df.index, df['DI+'], label='DI+', color='green', linewidth=1.5)
    plt.plot(df.index, df['DI-'], label='DI-', color='red', linewidth=1.5)
    plt.plot(df.index, df['ADX'], label='ADX', color='blue', linewidth=1.5)

    # Mark Buy and Sell signals
    plt.scatter(df.index[df['Buy Signal']], df['DI+'][df['Buy Signal']], marker='^', color='green', label='Buy Signal', s=100)
    plt.scatter(df.index[df['Sell Signal']], df['DI-'][df['Sell Signal']], marker='v', color='red', label='Sell Signal', s=100)
